...
const useGenerateContract = (address, enabled) => {
  ...
  const hasAllowance = async erc20PredicateAddress => {
    ...
    const response = await generatedContract.allowance(
      // Owner
      context.account,
      // Spender
  ...
  const estimateApprove = async erc20PredicateAddress => {
  ...
  const estimateTransferWithDepositOverhead = async (amount, vault) => {
    ...
    const withManagerOverhead = estimatedGas.add(
      transactionCosts.eth.rootManagerDepositOverheadGas,
    );
  ...
  const approve = async (erc20PredicateAddress: string) => {
    ...
    return await generatedContract.approve(
      // Spender
      erc20PredicateAddress,
      // Amount
      amountToApprove,
   ...
   */
  const checkAllowance = async erc20PredicateAddress => {
